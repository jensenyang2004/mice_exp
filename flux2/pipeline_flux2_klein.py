...
from .transformer_flux2_klein import Flux2Transformer2DModel
from .pipeline_utils import find_inner_sentence_token_span_qwen, create_position_mask_list, resize_mask
...
class Flux2KleinPipeline(DiffusionPipeline, Flux2LoraLoaderMixin):
    ...
    def _get_qwen3_prompt_embeds(
        ...
        prompt_settings: str = None,
        num_instance_text_tokens: int = 200,
        num_global_text_tokens: int = 200,
    ):
        assert prompt_settings in ['base', 'inner_local_prompts', 'outer_local_prompts', 'outer_global_prompts'], 'prompt_settings must be one of base, inner_local_prompts, outer_local_prompts, outer_global_prompts'
        ...
        for single_prompt in prompt_to_encode:
        ...
        # Initialize seq_len for the base case
        # seq_len = out.shape[2]
        ...
        if prompt_settings in ['inner_local_prompts', 'base']:
            ...
            for batch_idx, inner_sentences in enumerate(batched_inner_sentences):
                ...
                for inner_sentence in inner_sentences:
                    inner_mask = torch.zeros_like(attention_mask[batch_idx])
                    start, end = find_inner_sentence_token_span_qwen(tokenizer, prompt_to_encode[batch_idx], inner_sentence, max_sequence_length=max_sequence_length)
                    if start is None:
            ...
            for i, _ in enumerate(prompt_to_encode):
                num_tokens_to_select = num_global_text_tokens
                if i != 0:
                    num_tokens_to_select = torch.sum(input_ids[i] != 0).item() if prompt_settings == 'outer_local_prompts_smart' else num_instance_text_tokens
                out_list.append(out[[i], :, :num_tokens_to_select, :].contiguous())
                instance_text_index = torch.arange(begin_idx, begin_idx+num_tokens_to_select).int()
                instance_text_index_lst.append(instance_text_index)
                begin_idx += num_tokens_to_select
        ...
        out = out.permute(0, 2, 1, 3).reshape(batch_size, seq_len, num_channels * hidden_dim)

        return out, batched_instance_text_index_lst, seq_len
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            prompt_embeds, batched_instance_text_index_lst, seq_len = self._get_qwen3_prompt_embeds(
        ...
        return prompt_embeds, text_ids, batched_instance_text_index_lst, seq_len
    ...
    def __call__(
        ...
        hard_image_attribute_binding_list_double = [],
        hard_image_attribute_binding_list_single = [],
        instance_bboxes_xyxy_normalized: list[list[float]] = None,
        instance_masks_yx: list[torch.Tensor] = None,
        bring_area_to_1024_squared: bool = True,
        ...
        assert hard_masking_steps != None, "hard masking steps need to be selected"
        assert relaxed_timesteps != None, "relaxed timestep behavior needs to be selected"
        ...
        assert instance_bboxes_xyxy_normalized is not None or instance_masks_yx is not None, "instance_bboxes_xyxy_normalized or instance_masks_yx must be provided"
        assert instance_bboxes_xyxy_normalized is None or instance_masks_yx is None, "instance_bboxes_xyxy_normalized and instance_masks_yx cannot be provided together"
        use_masks = instance_masks_yx is not None
        ...
        prompt_embeds, text_ids, instance_text_index_lst, seq_len = self.encode_prompt(
            prompt=prompt,
            prompt_embeds=prompt_embeds,
            device=device,
        ...
        if self.do_classifier_free_guidance:
            ...
            negative_prompt_embeds, negative_text_ids, negative_instance_text_index_lst, negative_seq_len = self.encode_prompt(
        ...
        if image is not None:
            ...
            for img in image:
                ...
                img = self.image_processor.preprocess(img, height=image_height, width=image_width, resize_mode="default")
        ...
        if use_masks:
            instance_position_mask_list = [resize_mask(instance_mask_yx, height // self.vae_scale_factor // 2, width // self.vae_scale_factor // 2) for instance_mask_yx in instance_masks_yx]
            context_image_position_mask_list = [resize_mask(instance_mask_yx, height // self.vae_scale_factor // 2, width // self.vae_scale_factor // 2) for instance_mask_yx in instance_masks_yx]
        else:
            instance_position_mask_list = create_position_mask_list(instance_bboxes_xyxy_normalized, height, width, self.vae_scale_factor)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if attention_setting == "full":
                    pass
                elif attention_setting.lower() in ["apita", "apitasm", "apitasmasym", "apitasmmin", "apitasmdiag", "apitasmkernel", "apitasmkernellatent", "apitasmkernelnonlap", "apitasmkernelnonlapstrict"] or attention_setting in ["APITA", "APITASM", "APITASMasym", "APITASMmin", "APITASMdiag", "APITASMkernel", "APITASMkernelLatent", "APITASMkernelNonLap", "APITASMkernelNonLapStrict", "apitasmkernelnonlap", "apitasmkernelnonlapstrict"]:
                    if self._attention_kwargs is None:
                        self._attention_kwargs = {}
                    self._attention_kwargs['pos_instance_text_index_lst'] = instance_text_index_lst
                    self._attention_kwargs['instance_position_mask_list'] = instance_position_mask_list
                    self._attention_kwargs['pos_seq_len'] = seq_len
                    self._attention_kwargs['hard_image_attribute_binding_list_double'] = hard_image_attribute_binding_list_double
                    self._attention_kwargs['hard_image_attribute_binding_list_single'] = hard_image_attribute_binding_list_single
                    self._attention_kwargs['num_inference_steps'] = num_inference_steps
                    self._attention_kwargs['image_w_instance_token_index_list'] =  [list(range(latents.shape[1]))]
                    self._attention_kwargs['image_w_instance_token_H_list'] = [height]
                    self._attention_kwargs['image_w_instance_token_W_list'] = [width]
                    self._attention_kwargs['context_image_w_instance_token_index_list'] = [list(range(image_latents.shape[1]))]
                    self._attention_kwargs["hard_masking_steps"] = hard_masking_steps
                    self._attention_kwargs["relaxed_timesteps"] = relaxed_timesteps
                    ...
                    if self.do_classifier_free_guidance:
                        self._attention_kwargs['neg_instance_text_index_lst'] = negative_instance_text_index_lst
                        self._attention_kwargs['neg_seq_len'] = negative_seq_len
                else:
                    raise NotImplementedError(f"Attention setting {attention_setting} not implemented")

                with self.transformer.cache_context("cond"):
                    self._attention_kwargs['is_conditional'] = True
                ...
                if self.do_classifier_free_guidance:
                    with self.transformer.cache_context("uncond"):
                        self._attention_kwargs['is_conditional'] = False
